Tea.context(function () {
	...
	this.useGlobalSMTP = (this.instance.params.useGlobal == true)
	if (!this.hasGlobalEmailSetting) {
		this.useGlobalSMTP = false
	}
	...
	this.success = function () {
	    let that = this
		teaweb.success("淇濆瓨鎴愬姛", function () {
	...
	this.changeName = function (name) {
		if (name.indexOf("鐭俊") > -1 || name.indexOf("閽夐拤") > -1 || name.indexOf("寰俊") > -1) {
	...
	this.changeEmailUsername = function () {
		...
		if (this.instance.params.username.indexOf("qq.com") > 0) {
			this.emailUsernameHelp = "锛?a href=\"https://service.mail.qq.com\" target='_blank'>QQ閭鐩稿叧璁剧疆甯姪</a>";
		} else if (this.instance.params.username.indexOf("163.com") > 0) {
			this.emailUsernameHelp = "锛?a href=\"https://help.mail.163.com/faqDetail.do?code=d7a5dc8471cd0c0e8b4b8f4f8e49998b374173cfe9171305fa1ce630d7f67ac22dc0e9af8168582a\" target='_blank'>缃戞槗閭鐩稿叧璁剧疆甯姪</a>";
		}
	};

	this.onToggleGlobalSMTP = function () {
		if (!this.useGlobalSMTP && this.mediaType == "email") {
			this.$delay(function () {
				this.$find("form input[name='emailSmtp']").focus();
			});
		}
	...
	this.removeWebHookHeader = function (index) {
		if (!window.confirm("纭畾瑕佸垹闄ゆHeader鍚楋紵")) {
			return;
		}
	...
	this.removeWebHookParam = function (index) {
		if (!window.confirm("纭畾瑕佸垹闄ゆ鍙傛暟鍚楋紵")) {
			return;
		}
