Tea.context(function () {
    this.telegramProxyScheme = "";

    this.mediaType = ""
    this.useGlobalSMTP = this.hasGlobalEmailSetting
    this.advancedOptionsVisible = true

    let that = this
    this.changeMediaType = function (media) {
        that.mediaType = media.type
        if (that.mediaType == "email" && !that.useGlobalSMTP) {
            that.$delay(function () {
                that.$find("form input[name='emailSmtp']").focus();
            });
        }
    }
