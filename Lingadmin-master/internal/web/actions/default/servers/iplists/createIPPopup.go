...
func (this *CreateIPPopupAction) RunPost(params struct {
	...
	// 校验IPList
	// GlobalListId constant not available - using -1 as placeholder
	var globalListId int64 = -1 // firewallconfigs.GlobalListId not available in EdgeCommon 1.0.0
	if params.ListId != globalListId {
			...
			for index, line := range lines {
				...
				if strings.Contains(line, "/") { // CIDR
					...
					batchIPs = append(batchIPs, &ipData{
						ipFrom: ip.String(),
						ipTo:   ipNet.IP.String(), // Use the network address
					})
