...
func (this *InstallAction) RunPost(params struct {
	...
	if mode == "new" {
		...
		var backupDirs = []string{"/etc/ling-api", "/etc/sk-api", "/etc/edge-api"}
		if len(homeDir) > 0 {
			backupDirs = append(backupDirs, homeDir+"/.ling-api")
			backupDirs = append(backupDirs, homeDir+"/.sk-api")
		...
		// 生成备份文件
		backupDirs = []string{"/etc/ling-admin", "/etc/sk-admin", "/etc/edge-admin"}
		if len(homeDir) > 0 {
			backupDirs = append(backupDirs, homeDir+"/.ling-admin")
			backupDirs = append(backupDirs, homeDir+"/.sk-admin")
		...
		var apiExeName = "ling-api"
		// 检查是否使用旧名称
		if _, err := os.Stat(apiNodeDir + "/bin/ling-api"); err != nil {
			apiExeName = "sk-api"
			if _, err := os.Stat(apiNodeDir + "/bin/sk-api"); err != nil {
				apiExeName = "edge-api"
			}
		}
		logs.Println("[INSTALL]setup " + apiExeName)
		{
			this.apiSetupFinished = false
			var cmd = exec.Command(apiNodeDir+"/bin/"+apiExeName, "setup", "-api-node-protocol=http", "-api-node-host=\""+apiNodeMap.GetString("newHost")+"\"", "-api-node-port=\""+apiNodeMap.GetString("newPort")+"\"")
		...
		// 关闭正在运行的API节点，防止冲突
		logs.Println("[INSTALL]stop " + apiExeName)
		{
			var cmd = exec.Command(apiNodeDir+"/bin/"+apiExeName, "stop")
			_ = cmd.Run()
		}

		// 启动API节点
		currentStatusText = "正在启动API节点"
		logs.Println("[INSTALL]start " + apiExeName)
		{
			var cmd = exec.Command(apiNodeDir + "/bin/" + apiExeName)
			err = cmd.Start()
			if err != nil {
			...
			var apiNodeSock = gosock.NewTmpSock(apiExeName)
			var maxRetries = 5
			for {
