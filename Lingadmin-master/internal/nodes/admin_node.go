...
func (this *AdminNode) startAPINode() {
	// 优先使用ling-api，兼容sk-api和edge-api
	...
	configPath := apiNodeDir + "/configs/api.yaml"
	...
	if err == nil {
		...
		paths := []string{}
		if len(homeDir) > 0 {
			paths = append(paths, homeDir+"/.ling-api/api.yaml")
			paths = append(paths, homeDir+"/.sk-api/api.yaml")
			paths = append(paths, homeDir+"/.edge-api/api.yaml")
		}
		paths = append(paths, "/etc/ling-api/api.yaml")
		paths = append(paths, "/etc/sk-api/api.yaml")
		...
		for _, path := range paths {
			...
			if err == nil {
				...
				if err == nil {
					// 确保目录存在
					_ = os.MkdirAll(apiNodeDir+"/configs", 0755)
					err = os.WriteFile(configPath, data, 0666)
					if err == nil {
						logs.Println("[NODE]recover '" + apiNodeDir + "/configs/api.yaml' from '" + path + "'")
						canStart = true
						break
					}
				}
	...
	dbPath := apiNodeDir + "/configs/db.yaml"
	...
	if err != nil && os.IsNotExist(err) {
		...
		paths := []string{}
		if len(homeDir) > 0 {
			paths = append(paths, homeDir+"/.ling-api/db.yaml")
			paths = append(paths, homeDir+"/.sk-api/db.yaml")
			paths = append(paths, homeDir+"/.edge-api/db.yaml")
		}
		paths = append(paths, "/etc/ling-api/db.yaml")
		paths = append(paths, "/etc/sk-api/db.yaml")
		...
		for _, path := range paths {
			...
			if err == nil {
				...
				if err == nil {
					...
					if err == nil {
						logs.Println("[NODE]recover '" + apiNodeDir + "/configs/db.yaml' from '" + path + "'")
						break
					}
				}
			}
	...
	if canStart {
		logs.Println("start " + apiExeName)
		cmd := exec.Command(apiNodeDir + "/bin/" + apiExeName)
		err = cmd.Start()
		if err != nil {
			logs.Println("[ERROR]start " + apiExeName + " failed: " + err.Error())
		} else {
