...
func LoadAPIConfig() (*APIConfig, error) {
	...
	if err != nil {
		return nil, os.NewSyscallError("parse api.yaml from '"+foundPath+"'", err)
	}

	// 验证配置完整性
	if len(config.RPC.Endpoints) == 0 {
		return nil, os.NewSyscallError("invalid api.yaml", os.ErrInvalid)
	}
	if len(config.NodeId) == 0 || len(config.Secret) == 0 {
		return nil, os.NewSyscallError("invalid api.yaml: nodeId or secret is empty", os.ErrInvalid)
	}
