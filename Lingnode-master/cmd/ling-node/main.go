...
func main() {
	...
	app.On("ip.drop", func() {
		var args = os.Args[2:]
		if len(args) == 0 {
			fmt.Println("Usage:", teaconst.ProcessName, "ip.drop IP [--timeout=SECONDS] [--async]")
			return
		}
		var ip = args[0]
	...
	app.On("ip.reject", func() {
		var args = os.Args[2:]
		if len(args) == 0 {
			fmt.Println("Usage:", teaconst.ProcessName, "ip.reject IP [--timeout=SECONDS]")
			return
		}
		var ip = args[0]
		if len(net.ParseIP(ip)) == 0 {
	...
	app.On("ip.close", func() {
		var args = os.Args[2:]
		if len(args) == 0 {
			fmt.Println("Usage:", teaconst.ProcessName, "ip.close IP")
			return
		}
		var ip = args[0]
		if len(net.ParseIP(ip)) == 0 {
	...
	app.On("ip.remove", func() {
		var args = os.Args[2:]
		if len(args) == 0 {
			fmt.Println("Usage:", teaconst.ProcessName, "ip.remove IP")
			return
		}
		var ip = args[0]
		if len(net.ParseIP(ip)) == 0 {
	...
	app.On("disk", func() {
		...
		if len(args) > 0 {
			...
			default:
				fmt.Println("Usage:", teaconst.ProcessName, "disk [speed]")
			}
		} else {
			fmt.Println("Usage:", teaconst.ProcessName, "disk [speed]")
		}
	...
	app.On("config", func() {
		...
		if configString == "config" {
			fmt.Println("Usage:", teaconst.ProcessName, "config '\nrpc.endpoints: [\"...\"]\nnodeId: \"...\"\nsecret: \"...\"\n'")
			return
		}
