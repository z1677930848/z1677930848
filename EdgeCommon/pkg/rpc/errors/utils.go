...
func HumanError(err error, endpoints []string, configFile string) (resultErr error, isConnError bool) {
	...
	case codes.Unavailable:
		return fmt.Errorf("RPC当前不可用：<br/>1、请确认当前节点（%s）配置中的地址（%s）是否已填写正确；<br/>2、请确保API节点已启动，并检查当前节点和API节点之间的网络连接是否正常；<br/>3、可尝试可用的API端点：%s<hr/>错误信息：%w", filepath.Base(configFile), configFile, strings.Join(endpoints, ", "), err), true
	}
